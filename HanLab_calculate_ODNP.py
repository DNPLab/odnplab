...
def get_powers(directory: str, powerfile: str, ExpNums: list, bufferVal: float):
    """
    Args:
        directory (str): A string of the odnp experiment folder. e.g. '../data/topspin/'
        powerfile (str): A string specifying which file, either 'power' or 't1_powers'
        ExpNums (list): Folder numbers for experiments, either the
            enhancement folders list or the T1 folders list
        bufferVal (float): number that aligns something,
            2.5 for 'power' and (20 * 2.5) for 't1_power'

    Returns:
        list: array of power values taken from the .mat files
    """
...
if __name__ == "__main__":
    ...
    """
    directory = "TEMPO_500uM/"  # path to data folder
    ...
    print(
        "ksigma = "
        + str(round(hydration_results["ksigma"], 2))
        + " (s-1M-1), standard dev = "
        + str(round(hydration_results["ksigma_stdd"], 4))
    )
    print("coupling factor = " + str(round(hydration_results["coupling_factor"], 5)))
    print("tcorr = " + str(round(hydration_results["tcorr"], 2)) + " ps")
    print(
        "Dlocal = "
        + str(round(hydration_results["Dlocal"] * 1e10, 2))
        + " x 10^-10 (m^2/s)"
    )
    print("-----------------------")
    print("-----------------------")
